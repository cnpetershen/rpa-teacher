class CorrectionEngine:
    ...
    INTENT_PATTERNS = {
        "user_login": {
            "expected_actions": ["click", "type", "click", "type", "click"],
            "min_steps": 5,
        ...
        for required in pattern["must_have"]:
            if required not in actions:
                corrections.append({
                    ...
                    "severity": "error",
                    "step_index": None,
                    "message": f"缺少必要动作：'{required}'。当前意图需要该类型操作",
                    "suggestion": f"请在合适位置添加 '{required}' 操作",
                })
        return corrections
    ...
    # ----------------------------------------------------------------
    @staticmethod
    def _check_expected_sequence(steps, pattern, actions, overall_intent):
        corrections = []
        expected = pattern.get("expected_actions")
        if expected is None:
            return corrections

        # 检查是否 type 出现在第一个 click 之前（未聚焦就输入）
        ...
        if first_type_idx is not None and first_click_idx is not None and first_type_idx < first_click_idx:
            corrections.append({
                ...
                "message": "输入操作出现在点击之前：可能未聚焦目标就键入内容",
                "suggestion": "请在输入前添加点击操作来聚焦目标控件",
            })

        # 对于登录意图，检查三击结构
        if overall_intent == "user_login" and len(actions) >= 5:
            if actions[0] != "click" or actions[2] != "click" or actions[4] != "click":
                corrections.append({
                    "type": "WRONG_ORDER",
                    "severity": "warning",
                    "step_index": None,
        ...
        if overall_intent in ("web_search", "web_navigation", "text_editing"):
            if len(actions) > 2:
    ...
    def _check_empty_input(steps):
        ...
        for i, s in enumerate(steps):
            if s.get("action") == "type" and not s.get("value", "").strip():
                corrections.append({
                    "type": "EMPTY_INPUT",
                    "severity": "error",
                    "step_index": i,
                    "message": f"第 {i} 步输入内容为空",
                    "suggestion": "请检查是否遗漏了输入内容，或删除不必要的输入步骤",
                })
        return corrections
